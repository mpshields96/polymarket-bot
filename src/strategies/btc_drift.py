...
def load_sol_drift_from_config() -> BTCDriftStrategy:
    """Build BTCDriftStrategy for SOL markets from config.yaml strategy.sol_drift section.

    SOL is ~3x more volatile than BTC/ETH, so min_drift_pct is scaled up accordingly
    to maintain the same signal quality bar. Falls back to sol_drift section, then
    ...
    config_path = PROJECT_ROOT / "config.yaml"
    if not config_path.exists():
        logger.warning("config.yaml not found, using SOL drift defaults")
        return BTCDriftStrategy(name_override="sol_drift_v1")

    with open(config_path) as f:
        cfg = yaml.safe_load(f)

    # Fall back to btc_drift params if sol_drift section absent
    s = cfg.get("strategy", {}).get("sol_drift") or cfg.get("strategy", {}).get("btc_drift", {})
    return BTCDriftStrategy(
        sensitivity=s.get("sensitivity", _DEFAULT_SENSITIVITY),
        min_edge_pct=s.get("min_edge_pct", _DEFAULT_MIN_EDGE_PCT),
        min_minutes_remaining=s.get("min_minutes_remaining", _DEFAULT_MIN_MINUTES_REMAINING),
